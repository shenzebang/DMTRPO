...
def main(args):
    ...
    for i_episode in count(1):
        ...
        # directions comparision before line search
        print('Episode {}. Computing natural gradient directions...'.format(i_episode))
        time_begin = time()
        stepdirs = conjugate_gradient_parallel(policy_net, states_list, pg,
                                               args.max_kl, args.cg_damping, args.cg_iter)
        hm_fullstep = np.array(stepdirs).mean(axis=0)
        hm_fullstep = torch.from_numpy(hm_fullstep)
        dm_fullstep = np.average(stepdirs, axis=0, weights=normalized_determinants)
        dm_fullstep = torch.from_numpy(dm_fullstep)
        trpo_fullstep = conjugate_gradient_global(policy_net, states_list, pg,
                                             args.max_kl, args.cg_damping, args.cg_iter)
        ...
        for (n_backtracks, stepfrac) in enumerate(0.5 ** np.arange(10)):
            new_losses = []
            kls = []
            xnew = prev_params + stepfrac * dm_fullstep
            for advantages, states, actions in zip(advantages_list, states_list, actions_list):
